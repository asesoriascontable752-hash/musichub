...
export default function Player() {
  ...
  const spotifyEmbed = isSpotify && currentSong?.sourceUrl
    ? getSpotifyEmbedUrl(currentSong.sourceUrl)
    : null
  ...
  const getPlayerUrl = useCallback(() => {
    ...
    if (currentSong.sourceType === 'spotify') return '' // handled via iframe
  ...
  return (
    <>
      {/* Spotify embed — floats above the player bar */}
      {spotifyEmbed && (
      ...
      <div className="h-24 bg-[#181818] border-t border-white/5 px-4 flex items-center gap-4 relative z-50">
        {/* Hidden ReactPlayer (non-Spotify only) */}
        {playerUrl && !isSpotify && (
        ...
        <div className="flex items-center gap-3 w-[30%] min-w-0">
          ...
          <div className="min-w-0">
            ...
            {isSpotify && (
              <p className="text-[10px] text-spotify-green">▶ Reproduciendo en Spotify</p>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col items-center gap-1 max-w-[40%]">
          <div className="flex items-center gap-4">
            ...
            {/* Play button — hidden for Spotify (controlled via iframe) */}
            {!isSpotify && (
            ...
            {isSpotify && (
              <div className="w-9 h-9 bg-spotify-green/20 rounded-full flex items-center justify-center border border-spotify-green/40" title="Controlado desde el reproductor de arriba">
          ...
          {/* Progress bar — only for non-Spotify */}
          {!isSpotify && (
          ...
          {isSpotify && (
            <p className="text-xs text-white/30 mt-1">Usa el reproductor de Spotify de arriba para controlar</p>
          )}
        </div>
